...
with DAG(
    ...
    task6 = WaitSensor(time_to_wait=timedelta(seconds=600.0), task_id="test-wait-sensor", dag=dag)
    task7 = TimeSensor(target_time=time(12, 0, 0, 0), task_id="test-time-sensor", dag=dag)
    task8 = DateTimeSensor(target_time=datetime.fromisoformat("2025-01-01T12:00:00"), task_id="test-datetime-sensor", dag=dag)
"""
        )
